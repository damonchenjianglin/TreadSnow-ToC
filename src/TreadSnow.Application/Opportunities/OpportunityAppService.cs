...
namespace TreadSnow.Opportunities
{
    ...
    /// 商机应用服务
    /// </summary>
    [Authorize(TreadSnowPermissions.Opportunities.Default)]
    public class OpportunityAppService : ApplicationService, IOpportunityAppService
    {
        ...
        /// 构造函数
        /// </summary>
        /// <param name="repository">商机仓储</param>
        /// <param name="accountRepository">客户仓储</param>
        ...
        public OpportunityAppService(IRepository<Opportunity, Guid> repository, IRepository<Account, Guid> accountRepository, IAsyncQueryableExecuter asyncExecuter, DataPermissionService dataPermissionService, IRepository<IdentityUser, Guid> userRepository, IRepository<Team, Guid> teamRepository)
        {
        ...
        /// <returns>商机DTO（含客户名称）</returns>
        public async Task<OpportunityDto> GetAsync(Guid id)
        {
            var opportunity = await _repository.GetAsync(id);
            var dto = ObjectMapper.Map<Opportunity, OpportunityDto>(opportunity);

            var account = await _accountRepository.FindAsync(opportunity.AccountId);
            dto.AccountName = account?.Name;
            var dtoList = new List<OpportunityDto> { dto };
        ...
        /// <summary>
        /// 获取商机分页列表（支持name模糊筛选 + 负责人筛选，关联查询客户名称）
        /// </summary>
        /// <param name="input">查询条件</param>
        /// <returns>分页结果（含客户名称）</returns>
        public async Task<PagedResultDto<OpportunityDto>> GetListAsync(GetOpportunityListDto input)
        {
            ...
            query = await _dataPermissionService.ApplyReadFilterAsync(query, "opportunity", x => x.OwnerId, x => x.OwnerTeamId);
            ...
            var dtos = ObjectMapper.Map<List<Opportunity>, List<OpportunityDto>>(items);

            var accountIds = items.Select(p => p.AccountId).Distinct().ToList();
            ...
            return new PagedResultDto<OpportunityDto>(totalCount, dtos);
        }

        /// <summary>
        /// 获取所有商机列表（不分页，用于导出，关联查询客户名称）
        /// </summary>
        /// <param name="name">名称模糊筛选（可选）</param>
        /// <returns>全量列表</returns>
        public async Task<List<OpportunityDto>> GetExportListAsync(string? name)
        {
            var queryable = await _repository.GetQueryableAsync();
            var query = queryable.AsQueryable();
        ...
        /// <summary>
        /// 创建商机（OwnerId不传则默认当前用户）
        /// </summary>
        /// <param name="input">创建DTO</param>
        /// <returns>创建后的商机DTO</returns>
        [Authorize(TreadSnowPermissions.Opportunities.Create)]
        public async Task<OpportunityDto> CreateAsync(CreateOpportunityDto input)
        {
            ...
            var opportunity = ObjectMapper.Map<CreateOpportunityDto, Opportunity>(input);
            opportunity.TenantId = CurrentTenant.Id;
            opportunity.OwnerId = input.OwnerId ?? CurrentUser.Id;
            await _repository.InsertAsync(opportunity);
        ...
        [Authorize(TreadSnowPermissions.Opportunities.Edit)]
        public async Task<OpportunityDto> UpdateAsync(Guid id, UpdateOpportunityDto input)
        {
            var opportunity = await _repository.GetAsync(id);
            var hasPermission = await _dataPermissionService.CheckWritePermissionAsync("opportunity", opportunity.OwnerId, opportunity.OwnerTeamId);
        ...
        /// </summary>
        /// <param name="id">商机Id</param>
        [Authorize(TreadSnowPermissions.Opportunities.Delete)]
        public async Task DeleteAsync(Guid id)
        {
            var opportunity = await _repository.GetAsync(id);
            var hasPermission = await _dataPermissionService.CheckDeletePermissionAsync("opportunity", opportunity.OwnerId, opportunity.OwnerTeamId);
        ...
        private async Task FillPermissionsAsync(List<OpportunityDto> dtos)
        {
            var records = dtos.Select(d => (d.OwnerId, d.OwnerTeamId)).ToList();
            var permissions = await _dataPermissionService.BatchCheckPermissionsAsync("opportunity", records);
        ...
        /// </summary>
        /// <param name="dtos">DTO列表</param>
        private async Task FillLookupNamesAsync(List<OpportunityDto> dtos)
        {
